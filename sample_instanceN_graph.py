...
def conv_instanceN_relu_model(x):
  w = _weight([2, 2, c, c])
  b = _weight([c])
  ...
  z, _, _ = nv_norms.fused_instance_norm(y, gamma, beta, data_format='NHWC')
  x = tf.nn.relu(z)
  y = tf.nn.conv2d(x, w, strides=(1, 1), padding='SAME', data_format='NHWC')
  z, _, _ = nv_norms.fused_instance_norm(y, gamma, beta, data_format='NHWC')
  out = tf.nn.relu(z)
...
print_op_graph(
    conv_instanceN_relu_model, (n, h, w, c),
    f"layout_pass_instanceN_{precision.name}_{socket.gethostname()}.png",
    ['layout'])
